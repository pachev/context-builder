...
def generate_project_tree(path, prefix="", ignore_hidden=True, gitignore_rules=None):
    ...
    for i, (entry, full_path, is_dir) in enumerate(filtered_entries):
        ...
        if is_dir:
            ...
            subtree = generate_project_tree(
                full_path, next_prefix, ignore_hidden, gitignore_rules
            )
...
class FileTreeModel(QStandardItemModel):
    ...
    def _load_directory_recursive(self, path, parent_item):
        ...
        for entry in entries:
            ...
            if not self.ignore_gitignore and should_ignore(
                full_path, self.gitignore_rules
            ):
                continue

            if self.ignore_patterns and any(
                fnmatch(entry, pattern) for pattern in self.ignore_patterns
            ):
                continue
            ...
            if (
                ...
                and not any(entry.endswith(ext) for ext in self.file_extensions)
            ):
                continue
        ...
        # Sort directories first, then files
        for entry, item, is_dir in sorted(
            dir_entries, key=lambda x: (not x[2], x[0].lower())
        ):
...
class ContextBuilder(QMainWindow):
    def __init__(self):
        ...
        self.include_project_tree = False
    ...
    def toggle_project_tree(self, state):
    ...
    def generate_output(self, file_paths, output_format):
        ...
        for path in file_paths:
            try:
                ...
                result.append(
                    f"# Warning: Skipping file {path} due to UnicodeDecodeError"
                )
